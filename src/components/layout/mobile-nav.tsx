    ...
    <>
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-4 dark:border-slate-800 dark:bg-slate-950 lg:hidden">
        ...
        <div className="flex items-center gap-1">
          <LanguageToggle />
          <ThemeToggle />
          <button onClick={() => setOpen(true)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800" aria-label="Open navigation">
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 flex w-72 flex-col bg-slate-900 shadow-2xl">
            <div className="flex h-14 items-center justify-between border-b border-white/5 px-4">
              <Logo light />
              <button onClick={() => setOpen(false)} className="rounded-lg p-2 text-slate-400 hover:text-white" aria-label="Close navigation">
                <X className="size-5" />
              </button>
            </div>
