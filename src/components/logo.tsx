...
export function Logo({ className, light = false }: { className?: string; light?: boolean }) {
  ...
  return (
    <div className={cn("flex items-center gap-2.5", className)}>
      <div className="relative flex size-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 via-blue-600 to-blue-900 shadow-sm">
      ...
      <div className="leading-tight">
        <div className={cn("text-[15px] font-bold tracking-tight", light ? "text-white" : "text-slate-900 dark:text-slate-100")}>
          {t("brand.name")} <span className="text-blue-500">{t("brand.ai")}</span>
        </div>
